  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-2xl border border-[var(--editable-border)] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.06)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--slot4-accent)]">
            ...
            <button type="button" className="rounded-xl border border-[var(--editable-border)] px-5 py-2.5 text-sm font-bold transition hover:shadow-md" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] pl-9 pr-3 text-sm outline-none transition focus:border-[var(--slot4-accent)]"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-2xl border border-[var(--editable-border)] bg-white p-5 transition hover:shadow-md">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-center gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--slot4-accent-soft)] text-sm font-bold text-[var(--slot4-accent)]">{(item.name[0] || 'G').toUpperCase()}</span>
                    <div>
                      <p className="font-semibold">{item.name}</p>
                      <p className="text-xs text-[var(--slot4-muted-text)]">{formatDate(item.createdAt)}</p>
                    </div>
                  </div>
                  {item.articleSlug ? (
          ...
          <section className="mt-8 rounded-2xl border border-dashed border-[var(--editable-border)] bg-white p-8 text-center">
            <h2 className="editable-display text-xl font-bold">No comments yet</h2>
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--editable-border)] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
            ...
            <div className="flex gap-2">
              <button type="button" className="rounded-xl border border-[var(--editable-border)] px-4 py-2 font-bold transition hover:shadow-md disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-xl border border-[var(--editable-border)] px-4 py-2 font-bold transition hover:shadow-md disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
