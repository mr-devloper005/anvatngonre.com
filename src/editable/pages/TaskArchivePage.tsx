...
const placeholder = 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=1200&q=82'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[#f6f8fc] text-[var(--tk-text)]">
        <header className="relative overflow-hidden bg-[#101828] text-white">
          <div className="pointer-events-none absolute -right-20 -top-40 h-[480px] w-[480px] rounded-full bg-[#2878f0]/25 blur-3xl" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 pb-20 pt-16 sm:pb-24 sm:pt-20 lg:px-8">
            <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.28em] text-[#83adff]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-accent)] opacity-50" />
              <span className="text-white/55">{label}</span>
            </div>
            <h1 className="editable-display mt-6 max-w-4xl text-balance text-[2.5rem] font-extrabold leading-[1.06] tracking-[-0.035em] sm:text-5xl lg:text-6xl">
            ...
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-semibold text-white/75">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mt-12 flex flex-col gap-4 rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-xl border border-white/20 bg-white pl-4 pr-10 text-sm font-semibold text-[#172033] outline-none transition focus:border-[#2878f0]"
                ...
                <button className="inline-flex h-11 items-center rounded-xl bg-[#2878f0] px-6 text-sm font-bold text-white transition hover:bg-[#1768df]">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              ...
              <span className="rounded-xl bg-[#101828] px-5 py-2.5 font-bold text-white">Page {page} of {pagination.totalPages || 1}</span>
  ...
  return (
    <div className={`mt-2.5 flex items-center gap-2 ${center ? 'justify-center' : ''}`}>
      <span className="inline-flex items-center gap-[3px]">
        {[0, 1, 2, 3, 4].map((i) => (
          <Star key={i} className={`h-4 w-4 ${i < filled ? 'fill-amber-400 text-amber-400' : 'fill-[var(--tk-line)] text-[var(--tk-line)]'}`} />
        ))}
      </span>
